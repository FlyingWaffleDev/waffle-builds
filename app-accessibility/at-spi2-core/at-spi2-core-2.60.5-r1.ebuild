...
RDEPEND="${DEPEND}
	dbus-broker? (
		...
		!systemd? ( sys-apps/dbus-broker-dispatch )
	)
	${PYTHON_DEPS}
"
BDEPEND="
...
PATCHES=(
	"${FILESDIR}/${PN}-2.60.5-dbus-broker-dispatch.patch"
)

multilib_src_configure() {
	python_setup

	local broker_dispatch=disabled
	if use dbus-broker && ! use systemd; then
		broker_dispatch=/usr/bin/dbus-broker-dispatch
	fi

	local emesonargs=(
		-Ddefault_bus=$(usex dbus-broker dbus-broker dbus-daemon)
		-Ddbus_broker_dispatch="${broker_dispatch}"
...
multilib_src_test() {
	# Avoid locales using commas as decimal separators and breaking some tests
